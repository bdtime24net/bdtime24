...
export default defineEventHandler(async (event) => {
  try {
    ...
    const existingPost = await prisma.newsCategory.findUnique({ where: { id } });
    if (!existingPost) {
      throw createError({ statusCode: 404, message: "Category id not found" });
    }

    // Perform the update
    const updatedPost = await prisma.newsCategory.update({
      where: { id },
      data: {
        ...validatedData.data,
      },
