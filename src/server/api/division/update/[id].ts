...
import { divisionUpdateSchema } from "../validation";
...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (!id) {
      throw createError({ statusCode: 400, message: "division ID is required" });
    }

    // Read request body
    const body = await readBody(event);
    const validatedData = divisionUpdateSchema.safeParse(body);
    ...
    const existingPost = await prisma.division.findUnique({ where: { id } });
    if (!existingPost) {
